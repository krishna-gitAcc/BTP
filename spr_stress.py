...
def spr_plat_with_hole(gauss_coords, coord, connect, stress, int_nodes, n_patches, patch, ms):
    nodal_stress = np.zeros((n_patches, 3))
    # print(len(nodal_stress))
    a = np.zeros((n_patches, 3, 3))
    for i in range(n_patches):
    ...
    comp = [0, 1, 2]
    nod_stress = np.zeros((len(sp), 1))
    spr_stress = np.zeros((2*(ms+1)*(ms+1)-(ms+1), 3))
    ...
    for j in range(len(comp)):
        ...
        for l in range(2*(ms+1)*(ms+1)-(ms+1)):
            if l in int_nodes:
                index = int_nodes.index(l)
                spr_stress[l][comp[j]] = nodal_stress[index][comp[j]]
            if l in sp:
